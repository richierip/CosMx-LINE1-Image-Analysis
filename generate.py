...
def getMinCutPatchHorizontal(block1, block2, blocksize, overlap):
	...
	'''
	err = (block1[:, -overlap:] - block2[:, :overlap])**2
...
def getMinCutPatchBoth(refBlockLeft, refBlockTop, patchBlock, blocksize, overlap):
	...
	'''
	err = (refBlockLeft[:, -overlap:] - patchBlock[:, :overlap])**2
	# maintain minIndex for 2nd row onwards and 
	minIndex = []
	E = [list(err[0])]
	for i in range(1, err.shape[0]):
	...
	# Reverse to find full path
	path = path[::-1]
	mask1 = np.zeros((blocksize, blocksize))
	for i in range(len(path)):
		mask1[i, :path[i]+1] = 1

	###################################################################
	## Now for vertical one
	err = (np.rot90(refBlockTop)[:, -overlap:] - np.rot90(patchBlock)[:, :overlap])**2
	# maintain minIndex for 2nd row onwards and 
	minIndex = []
	E = [list(err[0])]
	for i in range(1, err.shape[0]):
	...
	# Reverse to find full path
	path = path[::-1]
	mask2 = np.zeros((blocksize, blocksize))
	for i in range(len(path)):
		mask2[i, :path[i]+1] = 1
...
def generateTextureMap(image, blocksize, overlap, outH, outW, tolerance):
	...
	textureMap = np.zeros(((blocksize + nH*(blocksize - overlap)), (blocksize + nW*(blocksize - overlap))))
